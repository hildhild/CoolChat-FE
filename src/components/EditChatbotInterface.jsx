...
export const EditChatbotInterface = ({toggleOpenChatbox, setToggleOpenChatbox, setChatboxConfig}) => {
  ...
  const handleChangeInput = (e) => {
    ...
    if (name === "main_name") {
      ...
      setEditConfigData({...editConfigData, message_border_radius: e.target.value});
    } else if (name === "message_background_color") {
      setEditConfigData({...editConfigData, message_background_color: e.target.value});
    } else if (name === "message_text_color") {
      setEditConfigData({...editConfigData, message_text_color: e.target.value});
    } 
  }
    ...
    <div>
      ...
      <div className="flex gap-8 mb-8">
        <div className="flex-col items-start flex">
          ...
          <Button isDisabled={!isEditable} className="border-[1px] border-gray-300" style={{backgroundColor: editConfigData.message_background_color}} onClick={()=>setIsSelectBgColor(!isSelectBgColor)}></Button>
          {
            isSelectBgColor
            &&
            <div>
              <div className="fixed top-0 left-0 right-0 bottom-0" onClick={ ()=>setIsSelectBgColor(false) }/>
              <ChromePicker className="z-20 absolute" color={editConfigData.message_background_color} onChange={(color)=>{setEditConfigData({...editConfigData, message_background_color: color.hex});}}/>
            </div>
          }
        </div>
        <div className="flex-col items-start flex">
          <div className="mb-2 text-center">Màu chữ tin nhắn nhận</div>
          <Button isDisabled={!isEditable} className="border-[1px] border-gray-300" style={{backgroundColor: editConfigData.message_text_color}} onClick={()=>setIsSelectTextColor(!isSelectTextColor)}></Button>
          {
            isSelectTextColor
            &&
            <div>
              <div className="fixed top-0 left-0 right-0 bottom-0" onClick={ ()=>setIsSelectTextColor(false) }/>
              <ChromePicker className="z-20 absolute" color={editConfigData.message_text_color} onChange={(color)=>{setEditConfigData({...editConfigData, message_text_color: color.hex});}}/>
            </div>
          }
        </div>
        <div className="flex-col items-start flex">
          <div className="mb-2 text-center">Màu nền tin nhắn gửi</div>
          <Button isDisabled={!isEditable} className="border-[1px] border-gray-300" style={{backgroundColor: editConfigData.message_background_color_me}} onClick={()=>setIsSelectBgColorMe(!isSelectBgColorMe)}></Button>
          {
            isSelectBgColorMe
            &&
            <div>
              <div className="fixed top-0 left-0 right-0 bottom-0" onClick={ ()=>setIsSelectBgColorMe(false) }/>
              <ChromePicker className="z-20 absolute" color={editConfigData.message_background_color_me} onChange={(color)=>{setEditConfigData({...editConfigData, message_background_color_me: color.hex});}}/>
            </div>
          }
        </div>
        <div className="flex-col items-start flex">
          <div className="mb-2 text-center">Màu chữ tin nhắn gửi</div>
          <Button isDisabled={!isEditable} className="border-[1px] border-gray-300" style={{backgroundColor: editConfigData.message_text_color_me}} onClick={()=>setIsSelectTextColorMe(!isSelectTextColorMe)}></Button>
          {
            isSelectTextColorMe
            &&
            <div>
              <div className="fixed top-0 left-0 right-0 bottom-0" onClick={ ()=> setIsSelectTextColorMe(false) }/>
              <ChromePicker className="z-20 absolute" color={editConfigData.message_text_color_me} onChange={(color)=>{setEditConfigData({...editConfigData, message_text_color_me: color.hex});}}/>
            </div>
          }
        </div>
      </div>
      {
        ...
        <div className="flex gap-5">
          <Button color="danger" onClick={handleCancel}>HỦY BỎ</Button>
          <Button color="primary" onClick={handlePreviewConfig}>XEM TRƯỚC</Button>
